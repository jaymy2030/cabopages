...
class pjAdminExtras extends pjAdmin
{
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['extra_create']))
			{
				...
				$id = pjExtraModel::factory(array_merge($_POST, $data))->insert()->getInsertId();
	...
	public function pjActionDeleteExtra()
	{
		...
		if ($this->isXHR())
		{
			...
			if (pjExtraModel::factory()->setAttributes(array('id' => $_GET['id']))->erase()->getAffectedRows() == 1)
			{
				...
				pjFleetExtraModel::factory()->where('extra_id', $_GET['id'])->eraseAll();
				$response['code'] = 200;
			} else {
				$response['code'] = 100;
			}
	...
	public function pjActionDeleteExtraBulk()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_POST['record']) && count($_POST['record']) > 0)
			{
				...
				pjFleetExtraModel::factory()->whereIn('extra_id', $_POST['record'])->eraseAll();
			}
		}
		exit;
	}
	...
	public function pjActionGetExtra()
	{
		...
		if ($this->isXHR())
		{
			...
			foreach($data as $k => $v)
			{
				$v['price'] = pjUtil::formatCurrencySign($v['price'], $this->option_arr['o_currency']) . ($v['per'] == 'person' ? ' ' . __('lblPerPerson', true) : '');
				$data[$k] = $v;
			}
