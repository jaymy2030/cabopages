...
class pjAdminClients extends pjAdmin
{
	...
	public function pjActionGetClient()
	{
		...
		if ($this->isXHR())
		{
			...
			$data = $pjClientModel
				->select("t1.*, AES_DECRYPT(t1.password, '".PJ_SALT."') AS `password`, (SELECT COUNT(`TB`.id) FROM `".pjBookingModel::factory()->getTable()."` AS `TB` WHERE `TB`.client_id=t1.id) cnt_bookings")
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['client_update']))
			{
				...
				$arr = pjClientModel::factory()
					->select("t1.*, AES_DECRYPT(t1.password, '".PJ_SALT."') AS `password`, (SELECT COUNT(TB.id) FROM `".$pjBookingModel->getTable()."` AS `TB` WHERE `TB`.client_id=t1.id) AS total_enquiries")
				...
				$enquiry_arr = $pjBookingModel->where('t1.client_id', $_GET['id'])->orderBy("created DESC")->limit(1)->findAll()->getData();
