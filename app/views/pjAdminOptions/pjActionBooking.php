...
if (isset($tpl['status']))
{
	...
	pjUtil::printNotice(__('infoReservationTitle', true), __('infoReservationDesc', true));
	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			{
				?>
				<form id="frmOptions" action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					if (in_array($tpl['arr'][$i]['key'], array('o_stripe_public_key', 'o_stripe_secret_key', 'o_stripe_cancel_url')))
					{
					    $rowClass = " boxStripe";
					    $rowStyle = "display: none";
								...
								case 'int':
									...
									{ 
										...
										<span class="pj-form-field-custom pj-form-field-custom-before">
											...
											<input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field w60 align_right pj-positive-number" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" />
										</span>
										<?php 
									} else { 
										?>
										<input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field field-int w60 pj-positive-number" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" />
										<?php
									} 
									if($tpl['arr'][$i]['key'] == 'o_hour_earlier'){
										__('lblOptionHours');
									}
									break;
								case 'float':
