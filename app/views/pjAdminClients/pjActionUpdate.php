...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminClients&amp;action=pjActionUpdate" method="post" id="frmUpdateClient" class="form pj-form">
		...
		<p>
			<label class="title"><?php __('lblTotalEnquiries'); ?></label>
			<span class="inline_block t5">
				<?php
				if((int) $tpl['arr']['total_enquiries'] > 0)
				{
					?><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionIndex&amp;client_id=<?php echo $tpl['arr']['id'];?>"><?php echo $tpl['arr']['total_enquiries'];?></a><?php
				}else{
					echo 0;
				} 
				?>
		...
		<?php
		if(count($tpl['enquiry_arr']) == 1)
		{
			?>
			<p>
				<label class="title"><?php __('lblLastEnquiry'); ?></label>
				<span class="inline_block t5">
					<a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionUpdate&amp;id=<?php echo $tpl['enquiry_arr'][0]['id'];?>"><?php echo date($tpl['option_arr']['o_date_format'], strtotime($tpl['enquiry_arr'][0]['created']));?></a>
				</span>
			</p>
			<?php
		} 
